 ...
 * route over.
 */
export async function ArticlePageImpl({
  params,
  pathSection,
}: ArticlePageProps & { pathSection: Section }) {
  ...
  /* The URL claims a section; the store knows the truth. A mismatch (a stale link after a
     reclassification, a hand-edited URL) redirects to the canonical path rather than rendering
     the story inside a world it does not belong to. */
  if ((article.section ?? "ai") !== pathSection) {
            ...
            <ul className="mt-2 flex flex-col gap-1.5">
              ...
              {siblings.map((sibling) => (
                <li key={sibling.urlHash}>
                  <Link
                    href={articlePath(sibling.section ?? null, sibling.urlHash)}
